...
import org.ethan.my8583.cnValue;
...
public class cnFieldParseInfo {
	...
	public cnFieldParseInfo(cnType t, int len, String encode) {
		if (t == null) {
	...
	public cnValue<?> parse(byte[] buf, int pos) throws ParseException { /// 解析报文
		try {
			if (type == cnType.NUMERIC || type == cnType.ALPHA) {
				return new cnValue<String>(type, new String(buf, pos, length, encode), length, encode); // 修改：指定字符串编码；
			} else if (type == cnType.LLVAR) {
				length = ((buf[pos] - 48) * 10) + (buf[pos + 1] - 48);
				return new cnValue<String>(type, new String(buf, pos + 2, length, encode), length, encode); // 修改：按照获取的length指定长度；指定字符串编码；
			} else if (type == cnType.LLLVAR) {
				length = ((buf[pos] - 48) * 100) + ((buf[pos + 1] - 48) * 10) + (buf[pos + 2] - 48);
				return new cnValue<String>(type, new String(buf, pos + 3, length, encode), length, encode); // 修改：按照获取的length指定长度；指定字符串编码；
				...
				return new cnValue<Date>(type, cal.getTime());
			} else if (type == cnType.BINARY) { // 追加，二进制解析；
				return new cnValue<byte[]>(type, Arrays.copyOfRange(buf, pos, pos + length), length, encode);
			} else if (type == cnType.LLBIN) { // 追加，二进制解析；
				length = ((buf[pos] - 48) * 10) + (buf[pos + 1] - 48);
				return new cnValue<byte[]>(type, Arrays.copyOfRange(buf, pos + 2, pos + 2 + length), length, encode); // 修改：按照获取的length指定长度；
			} else if (type == cnType.LLLBIN) { // 追加，二进制解析；
				length = ((buf[pos] - 48) * 100) + ((buf[pos + 1] - 48) * 10) + (buf[pos + 2] - 48);
				return new cnValue<byte[]>(type, Arrays.copyOfRange(buf, pos + 3, pos + 3 + length), length, encode); // 修改：按照获取的length指定长度；
			}
		} catch (UnsupportedEncodingException e) {
			throw new ParseException("字符串编码错误.", pos);
		}
		return null;
	}
	 ...
	 * type and length. */
	public cnValue<?> parseBinary(byte[] buf, int pos) throws ParseException { /// 解析二进制报文
		try {
			if (type == cnType.ALPHA) {
				return new cnValue<String>(type, new String(buf, pos, length, encode), length, encode);
				...
				return new cnValue<String>(type, new String(buf, pos + 1, length, encode), length, encode); // 修改：按照获取的length指定长度；
	
			} else if (type == cnType.LLLVAR) {
				length = ((buf[pos] & 0x0f) * 100) + (((buf[pos + 1] & 0xf0) >> 4) * 10) + (buf[pos + 1] & 0x0f);
				return new cnValue<String>(type, new String(buf, pos + 2, length, encode), length, encode); // 修改：按照获取的length指定长度；
				...
				return new cnValue<Date>(type, cal.getTime());
			} else if (type == cnType.BINARY) { // 追加
				return new cnValue<byte[]>(type, Arrays.copyOfRange(buf, pos, pos + length), length, encode);
			} else if (type == cnType.LLBIN) { // 追加
				length = (((buf[pos] & 0xf0) >> 4) * 10) + (buf[pos] & 0x0f);
				return new cnValue<byte[]>(type, Arrays.copyOfRange(buf, pos + 2, pos + 2 + length), length, encode); // 修改：按照获取的length指定长度； 
			} else if (type == cnType.LLLBIN) { // 追加
				length = ((buf[pos] & 0x0f) * 100) + (((buf[pos + 1] & 0xf0) >> 4) * 10) + (buf[pos + 1] & 0x0f);
				return new cnValue<byte[]>(type, Arrays.copyOfRange(buf, pos + 3, pos + 3 + length), length, encode); // 修改：按照获取的length指定长度；
			}
		} catch (UnsupportedEncodingException e) {
