...
import org.ethan.my8583.parse.cnFieldParseInfo;
import org.etyhan.my8583.util.StringUtil;

/**
 * 这是一个中国版的8583格式标准的类，初始代码来源于MessageFactory类。 This class is used to create
 * messages, either from scratch or from an existing String or byte buffer. It
 * can be configured to put default values on newly created messages, and also
 * to know what to expect when reading messages from an InputStream.
 * <P>
...
public class cnMessageFactory {
	 ...
	 * 创建cnMessage
	 */
	public cnMessage newMessage(String msgtypeid, int headerlength) {
		return new cnMessage(msgtypeid, headerlength, encode);
	}
	
	/**
	 * 创建cnMessage
	 */
	public cnMessage newMessage() {
		return new cnMessage(encode);
	}

	/**
	 * Creates a new message of the specified type id from message template. If
	...
	public cnMessage newMessagefromTemplate(String msgtypeid) {
		cnMessage m = new cnMessage(msgtypeid, msgheadersattr.get(msgtypeid), encode);
		m.setEtx(etx);
		...
		//设置bitMap;
		m.setBitMap(new BitMap(fieldIndexList));
	 ...
	 * <P/>
	 * sets all fields from the template if there is one, and copies all values
	 * from the request, overwriting fields from the template if they overlap.
	 * 
	 * @param request An 8583 message with a request type (ending in 00).
	 */
	public cnMessage createResponse(cnMessage request) {
		String resptypeid = request.getMsgTypeID().substring(0, 2) + Integer.toString(Integer.parseInt(request.getMsgTypeID().substring(2, 3)) + 1)
				+ request.getMsgTypeID().substring(3, 4);
		cnMessage resp = new cnMessage(resptypeid, msgheadersattr.get(resptypeid), encode);
	...
	public cnMessage parseMessage(InputStream in, int headerLength) throws ParseException, IOException {
		byte[] temp = new byte[headerLength];
		in.read(temp);	// 读46个字节的数据（将报文头信息读出）
		
		Head header = new Head(temp);
		System.out.println("Header: "+header.getAsStr());
		
		temp = new byte[Integer.valueOf(new String(header.getTotalMsgLength()))];
		in.read(temp);	// 从第46个字节读取len个自己的数据到buf中
		in.close();
		
		setUseBinary(false);
		cnMessage m = parseMessage(temp, headerLength);	// 解析
		m.setHeader(header);
		return m;
	}
	 ...
	 *            length header.
	 * @param msgheaderlength The expected length of the 8583 header, after
	 *            which the message type id and the rest of the message must
	 *            come.
	 */
	public cnMessage parseMessage(byte[] buf, int msgheaderlength) throws ParseException {
		cnMessage m = new cnMessage(new String(buf, msgheaderlength, 4), msgheaderlength, encode);
		// TODO it only parses ASCII messages for now

		// 得到报文头
//		m.setMessageHeaderData(0, Arrays.copyOfRange(buf, 0, msgheaderlength));
//		m.setHeader(new Head(Arrays.copyOfRange(buf, 0, msgheaderlength)));
		...
		for (Integer i : index) {
			...
			if (bs.get(i - 1)) {
				...
				if (useBinary && !(val.getType() == cnType.ALPHA || val.getType() == cnType.LLVAR || val.getType() == cnType.LLLVAR
						|| val.getType() == cnType.BINARY || val.getType() == cnType.LLBIN || val.getType() == cnType.LLLBIN)) {
					pos += (val.getLength() / 2) + (val.getLength() % 2);
				} else {
					pos += val.getLength();
				}
				if (val.getType() == cnType.LLVAR || val.getType() == cnType.LLBIN) { // 追加
					pos += useBinary ? 1 : 2;
				} else if (val.getType() == cnType.LLLVAR || val.getType() == cnType.LLLBIN) { // 追加
					pos += useBinary ? 2 : 3;
				}
			}
		}
	 ...
	 * 
	 * @param value A map where the keys are the message type id and the values
	 ...
	 * same message type id as the new one, it is overwritten.
	 */
	...
	/**
	 * Sets a message template for a specified message type. When new messages
	 * of that type are created, they will have the same values as the template.
	 * 
	 * @param msgtypeid The message type id.
	 * @param templ The message from which fields should be copied, or NULL to
	 *            remove the template for this message type.
	 * @deprecated Use addMessageTemplate(cnMessage) and
	 *             removeMessageTemplate(String) instead of this.
	 */
	...
	public Head createHeader(cnMessage message) {
		return new Head().setHeaderLength((byte)46)
				.setFlagAndVersion((byte)10)
				.setTotalMsgLength(StringUtil.align(message.estimatetotalmsglength()+"", 4, true, "0").getBytes())
				.setDestinationStationId(StringUtil.align("00010000", 11, false, " "))
				.setSourceStationId(StringUtil.align("00054000", 11, false, " "))
				.setReserverForUse(new byte[3])
				.setBatchNumber((byte)0)
				.setTransactionInformation("00000000".getBytes())
				.setUserInformation((byte)0)
